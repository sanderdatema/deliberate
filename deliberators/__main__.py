...
def main() -> None:
    ...
    if not os.environ.get("DELIBERATORS_ALLOW_CLI"):
        print(
            "\n"
            "⛔  De CLI is uitgeschakeld voor gebruik vanuit Claude Code.\n"
            "\n"
            "Gebruik de /deliberate skill in plaats van de CLI:\n"
            "  /deliberate \"jouw vraag\"\n"
            "  /deliberate --preset deep \"jouw vraag\"\n"
            "  /deliberate --files src/foo.py \"review this\"\n"
            "\n"
            "De skill orchestreert het team in-context, toont team-goedkeuring,\n"
            "en geeft het rapport direct in de conversatie terug.\n"
            "\n"
            "Wil je de CLI tóch gebruiken (bijv. voor testing):\n"
            "  DELIBERATORS_ALLOW_CLI=1 uv run python -m deliberators ...\n",
            file=sys.stderr,
        )
        sys.exit(1)
