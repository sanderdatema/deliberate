...
# Allow CLI usage in tests
_CLI_ENV = {**os.environ, "DELIBERATORS_ALLOW_CLI": "1"}
...
class TestCLIHelp:
    ...
    def test_cli_blocked_without_env_var(self) -> None:
        """CLI should exit 1 with a helpful message when DELIBERATORS_ALLOW_CLI is not set."""
        env = {k: v for k, v in os.environ.items() if k != "DELIBERATORS_ALLOW_CLI"}
        result = subprocess.run(
            [sys.executable, "-m", "deliberators", "test question"],
            capture_output=True,
            text=True,
            env=env,
        )
        ...
        assert "DELIBERATORS_ALLOW_CLI" in result.stderr
