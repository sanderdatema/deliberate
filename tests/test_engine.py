    ...
    return Config(
        ...
        presets={
            "quick": Preset(
                ...
                team_size=3,
                editor_count=2,
                analysts=("occam", "socrates", "fowler"),
            ...
            "balanced": Preset(
                ...
                team_size=5,
                editor_count=4,
                analysts=("socrates", "occam", "fowler", "schneier", "don-norman"),
            ...
            "deep": Preset(
                ...
                team_size=8,
                editor_count=4,
                analysts=("socrates", "occam", "fowler", "schneier", "don-norman", "linus", "kent-beck", "lupin"),
                editors=("marx", "hegel", "arendt", "samenvatter"),
                summarizer="samenvatter",
            ),
        },
...
class TestBalancedPreset:
    ...
    @pytest.mark.asyncio
    async def test_correct_agent_count(self, config: Config, personas: dict[str, Persona]) -> None:
        """AC-1: balanced = 5 analysts x 2 rounds + 4 editors (no convergence: min==max)."""
        tracker = make_mock_subprocess()
        with patch("deliberators.engine.asyncio.create_subprocess_exec", new=AsyncMock(side_effect=tracker)):
            engine = DeliberationEngine(config, personas)
            await engine.run("Test question", "balanced")

        assert tracker.call_count == 16  # 1 intake + 5 R1 + 5 R2 + 4 editors + 1 synthesis
...
class TestRound2FullOutput:
    ...
    async def test_round2_contains_full_round1(self, config: Config, personas: dict[str, Persona]) -> None:
        ...
        # Calls 7-11 are Round 2 (call 0=intake, calls 1-5=R1, calls 6-10=R2)
    ...
    async def test_round2_prompt_not_lossy(self, config: Config, personas: dict[str, Persona]) -> None:
        ...
        # Check a Round 2 call has the full 500-char output (index 6: after intake+R1)
...
class TestAgentErrorHandling:
    ...
    async def test_failed_agent_returns_error_string(self, config: Config, personas: dict[str, Persona]) -> None:
        ...
        assert result.rounds[1]["socrates"] != ""
        assert "[Agent fout:" not in result.rounds[1]["socrates"]
        assert result.rounds[1]["fowler"] != ""
...
class TestInjectableAgentFn:
    ...
    async def test_agent_fn_receives_correct_persona(
        ...
        # Quick preset: occam, socrates, fowler (analysts) + marx, samenvatter (editors)
        assert "Willem van Occam" in received_personas
        assert "Socrates" in received_personas
        assert "Martin Fowler" in received_personas
    ...
    async def test_concurrent_agents_limited(self, personas: dict[str, Persona]) -> None:
        ...
        limited_config = Config(
            ...
            presets={
                "test": Preset(
                    name="test", description="", max_rounds=1,
                    analysts=("occam", "socrates", "fowler", "schneier", "don-norman"),
                    editors=("samenvatter",),
                ),
            },
...
class TestParseKvLines:
    """Unit tests for the shared _parse_kv_lines helper."""

    def test_parses_matching_fields(self) -> None:
        from deliberators.engine import _parse_kv_lines
        output = "FOO: bar\nBAZ: qux"
        parsed, parsed_any = _parse_kv_lines(output, {"FOO": "foo", "BAZ": "baz"}, "Test")
        assert parsed == {"foo": "bar", "baz": "qux"}
        assert parsed_any is True

    def test_case_insensitive_prefix(self) -> None:
        from deliberators.engine import _parse_kv_lines
        output = "foo: bar"
        parsed, parsed_any = _parse_kv_lines(output, {"FOO": "foo"}, "Test")
        assert parsed == {"foo": "bar"}
        assert parsed_any is True

    def test_no_match_returns_empty(self, caplog: pytest.LogCaptureFixture) -> None:
        from deliberators.engine import _parse_kv_lines
        import logging
        with caplog.at_level(logging.WARNING, logger="deliberators.engine"):
            parsed, parsed_any = _parse_kv_lines("garbage", {"FOO": "foo"}, "Test")
        assert parsed == {}
        assert parsed_any is False
        assert any("Test: no fields parsed" in r.message for r in caplog.records)

    def test_empty_output(self, caplog: pytest.LogCaptureFixture) -> None:
        from deliberators.engine import _parse_kv_lines
        import logging
        with caplog.at_level(logging.WARNING, logger="deliberators.engine"):
            parsed, parsed_any = _parse_kv_lines("", {"FOO": "foo"}, "Test")
        assert parsed == {}
        assert parsed_any is False

    def test_preserves_value_after_colon(self) -> None:
        from deliberators.engine import _parse_kv_lines
        output = "KEY: value with: colons"
        parsed, _ = _parse_kv_lines(output, {"KEY": "key"}, "Test")
        assert parsed["key"] == "value with: colons"
...
class TestConvergencePhase:
    ...
    @pytest.mark.asyncio
    async def test_no_convergence_for_balanced(self, config: Config, personas: dict[str, Persona]) -> None:
        """Balanced preset (min==max==2) skips convergence entirely."""
        events: list[DeliberationEvent] = []
    ...
    @pytest.mark.asyncio
    async def test_convergence_events_emitted_for_deep(self, config: Config, personas: dict[str, Persona]) -> None:
        """Deep preset emits convergence events after R2 (min_rounds=2)."""
        events: list[DeliberationEvent] = []
        ...
        with patch("deliberators.engine.asyncio.create_subprocess_exec", new=AsyncMock(side_effect=tracker)):
            engine = DeliberationEngine(config, personas, on_event=collect)
            await engine.run("Test question", "deep")

        types = [e.type for e in events]
        assert "convergence_started" in types
        assert "convergence_completed" in types
        # Convergence should come after second round_completed, before third round_started
        ...
        assert conv_idx > round_completed_indices[1]  # after second round completes
        assert conv_idx < round_started_indices[2]  # before third round starts
...
class TestParseTeamSelectionOutput:
    ...
    def test_parse_valid_output(self, mock_personas: dict[str, Persona]) -> None:
        ...
        analysts, editors, reason, _ = DeliberationEngine._parse_team_selection_output(output, mock_personas)
    ...
    def test_parse_empty_output(self, mock_personas: dict[str, Persona]) -> None:
        analysts, editors, reason, _ = DeliberationEngine._parse_team_selection_output("", mock_personas)
        assert analysts == []
        assert editors == []
        assert reason == ""
    ...
    def test_parse_motivations(self, mock_personas: dict[str, Persona]) -> None:
        output = (
            "ANALYSTS: socrates, occam\nEDITORS: marx\nREASON: Good team\n"
            "MOTIVATIONS: socrates: dialectical depth | occam: simplification lens | marx: structural critique"
        )
        _, _, _, motivations = DeliberationEngine._parse_team_selection_output(output, mock_personas)
        assert motivations["socrates"] == "dialectical depth"
        assert motivations["occam"] == "simplification lens"
        assert motivations["marx"] == "structural critique"

    def test_empty_motivations(self, mock_personas: dict[str, Persona]) -> None:
        output = "ANALYSTS: socrates\nEDITORS: marx\nREASON: test"
        _, _, _, motivations = DeliberationEngine._parse_team_selection_output(output, mock_personas)
        assert motivations == {}


class TestTeamApproval:
    """Tests for the on_approve_team callback."""

    @pytest.mark.asyncio
    async def test_approval_callback_called(self, config: Config, personas: dict[str, Persona]) -> None:
        """on_approve_team is called with team details after dynamic selection."""
        approval_calls: list[tuple] = []

        async def approve(analysts, editors, reason, motivations):
            approval_calls.append((analysts, editors, reason, motivations))
            return analysts, editors  # approve as-is

        # Use a config without fixed analyst/editor lists to trigger dynamic selection
        dynamic_config = Config(
            ...
            presets={
                "test": Preset(
                    ...
                    team_size=3, editor_count=1, summarizer="samenvatter",
                ),
            },
        )

        tracker = make_mock_subprocess(response_map={
            "team selection": "ANALYSTS: socrates, occam, fowler\nEDITORS: samenvatter\nREASON: Good fit",
        })
        with patch("deliberators.engine.asyncio.create_subprocess_exec", new=AsyncMock(side_effect=tracker)):
            engine = DeliberationEngine(dynamic_config, personas, on_approve_team=approve)
            await engine.run("Test", "test")

        assert len(approval_calls) == 1

    @pytest.mark.asyncio
    async def test_approval_can_modify_team(self, config: Config, personas: dict[str, Persona]) -> None:
        """on_approve_team can swap out team members."""
        async def swap_team(analysts, editors, reason, motivations):
            return ["occam", "socrates"], ["samenvatter"]
        ...
        with patch("deliberators.engine.asyncio.create_subprocess_exec", new=AsyncMock(side_effect=tracker)):
            engine = DeliberationEngine(dynamic_config, personas, on_approve_team=swap_team)
            result = await engine.run("Test", "test")

        # Only the 2 approved analysts should appear in results
        assert set(result.rounds[1].keys()) == {"occam", "socrates"}

    @pytest.mark.asyncio
    async def test_no_approval_callback_skips_approval(self, config: Config, personas: dict[str, Persona]) -> None:
        """Without on_approve_team, team selection proceeds without approval."""
        tracker = make_mock_subprocess()
        with patch("deliberators.engine.asyncio.create_subprocess_exec", new=AsyncMock(side_effect=tracker)):
            engine = DeliberationEngine(config, personas)
            result = await engine.run("Test", "quick")
