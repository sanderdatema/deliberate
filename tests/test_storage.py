...
from deliberators.models import DecisionRecord, DeliberationResult, Preset
from deliberators.storage import DecisionStore, to_decision_record
...
class TestToDecisionRecord:
    """Tests for to_decision_record conversion from DeliberationResult."""

    def _make_result(self) -> DeliberationResult:
        return DeliberationResult(
            question="Test question",
            preset=Preset(name="balanced", description="", max_rounds=2),
            rounds={1: {"socrates": "Analysis A", "occam": "Analysis B"}},
            editor_outputs={"marx": "Editorial"},
            samenvatter_output="Summary here",
        )

    def test_creates_record_with_correct_fields(self) -> None:
        result = self._make_result()
        record = to_decision_record(result)
        assert record.question == "Test question"
        assert record.preset_name == "balanced"
        assert record.analysts == ("socrates", "occam")
        assert record.editors == ("marx",)
        assert record.summary == "Summary here"
        assert record.follow_up_of is None
        assert len(record.id) == 32  # uuid hex

    def test_follow_up_of_set(self) -> None:
        result = self._make_result()
        record = to_decision_record(result, follow_up_of="parent123")
        assert record.follow_up_of == "parent123"

    def test_condenses_positions(self) -> None:
        result = self._make_result()
        record = to_decision_record(result)
        assert "socrates" in record.key_positions
        assert "occam" in record.key_positions

    def test_empty_rounds(self) -> None:
        result = DeliberationResult(
            question="Empty",
            preset=Preset(name="q", description="", max_rounds=1),
        )
        record = to_decision_record(result)
        assert record.analysts == ()
        assert record.key_positions == {}
